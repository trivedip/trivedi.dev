...
function Header() {   
    ...
    return(
        <>
                <div>
                    <span className="sm:hidden z-20 absolute right-0 py-12 z-50 max-w-screen-xl focus:outline-none" onClick={toggleMenu}>
                        <BiMenu className="w-8 h-8" style={{transform:'scale(1.8)'}} />
                    </span>
                </div>
                <div className={menu ? "container m-auto sm:px-12 px-5 max-w-screen-xl  h-32 overflow-hidden":"container m-auto max-w-screen-xl fixer"}>
                    <nav className="flex space-x-6 items-center justify-center md:justify-start text-sm sm:flex flex-col md:flex-row">
                        ...
                        <div className={menu ? "hidden sm:flex flex-grow items-center space-x-6":"items-center menu-open sm:flex w-full"}>
                            {pages.map((page)=>(
